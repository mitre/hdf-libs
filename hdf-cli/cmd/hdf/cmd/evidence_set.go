...
Examples:
  ...
  hdf evidence set evidence.json --unset description -o updated.json`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			if name == "" && description == "" && packageID == "" && systemRef == "" && docVersion == "" && len(unsetFields) == 0 {
				return fmt.Errorf("at least one field flag is required (--name, --description, --package-id, --system-ref, --version, --unset)")
			}
