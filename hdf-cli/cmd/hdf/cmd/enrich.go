...
Enrichment is INFORMATIONAL: it adds context and never changes a finding's
status or impact. The source format is auto-detected; assert it with --from.
Supported sources: stix (a STIX 2.1 bundle).
...
Examples:
  ...
  hdf enrich results.json bundle.json                       # write to stdout`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			return runEnrich(args[0], args[1], fromFormat, outputPath)
		},
	}
...
func runEnrich(resultsPath, sourcePath, fromFormat, outputPath string) error {
	...
	enriched, err := shared.EnrichStix(resultsData, sourceData)
	if err != nil {
