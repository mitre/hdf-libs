...
func TestListSystemDataFlows(t *testing.T) {
	const (
		comp1 = "11111111-1111-1111-1111-111111111111"
		comp2 = "22222222-2222-2222-2222-222222222222"
	)
	sysJSON := `{
		"name": "Portal-Prod",
		"components": [
			{"name": "WebTier", "type": "application", "componentId": "` + comp1 + `"},
			{"name": "DB", "type": "database", "componentId": "` + comp2 + `"}
		],
		"dataFlows": [
			{"from": "` + comp1 + `", "to": "` + comp2 + `", "protocol": "JDBC", "port": 5432, "description": "Web to database"},
			{"from": "` + comp2 + `", "to": "` + comp1 + `", "protocol": "JDBC", "description": "Query results"}
		]
	}`
	...
	t.Run("detail dataFlows shows flows", func(t *testing.T) {
		...
		assert.Contains(t, stdout, comp1)
		assert.Contains(t, stdout, comp2)
