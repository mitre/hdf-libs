...
func TestGuardFileSize(t *testing.T) {
	...
	if terr := guardFileSize(p, 100); terr == nil || terr.Code != mcperr.TooLarge {
		t.Fatalf("300-byte file over a 100 limit must be TOO_LARGE, got %v", terr)
	}
	if terr := guardFileSize(p, 1000); terr != nil {
		t.Fatalf("300-byte file under a 1000 limit must pass, got %v", terr)
	}
	if terr := guardFileSize(filepath.Join(root, "nope.json"), 1000); terr == nil || terr.Code != mcperr.DocumentNotFound {
