...
// computation (impact==0 → notApplicable unless the roll-up is error,
// governing override, stored effectiveStatus, worst-wins rollup). It returns the schema (effectiveStatus)
...
// effective status (impact==0 → notApplicable unless the roll-up is error,
// overrides honored) — the single
