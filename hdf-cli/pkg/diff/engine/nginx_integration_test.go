...
func TestNginx_V1FormatDetection(t *testing.T) {
	...
	if !normalize.IsV1Format(nginx.beforeRaw) {
		t.Error("nginx-failing.json should be detected as v1 format")
	}
	if !normalize.IsV1Format(nginx.afterRaw) {
...
func TestNginx_FixedHaveResultChanged(t *testing.T) {
	...
	fixedReqs := filterByState(nginx.diff.RequirementDiffs, types.StateFixed)
	if len(fixedReqs) == 0 {
...
func TestNginx_FormatVersionAndComparisonMode(t *testing.T) {
	...
	if nginx.diff.ComparisonMode != types.ModeTemporal {
		t.Errorf("comparisonMode = %q, want %q", nginx.diff.ComparisonMode, types.ModeTemporal)
	}
}
