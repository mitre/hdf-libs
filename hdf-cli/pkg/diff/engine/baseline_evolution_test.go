...
func TestDiffBaselines_ComparisonMode(t *testing.T) {
	...
	if result.ComparisonMode != types.ModeBaselineEvolution {
		t.Errorf("expected comparisonMode %q, got %q", types.ModeBaselineEvolution, result.ComparisonMode)
	}
}
...
func TestDiffBaselines_BaselineDiffs(t *testing.T) {
	...
	if bd.State != types.StateUpdated {
		t.Errorf("expected baseline state %q, got %q", types.StateUpdated, bd.State)
	}
}
...
func TestDiffBaselines_BaselineDiffsUnchanged(t *testing.T) {
	...
	if result.BaselineDiffs[0].State != types.StateUnchanged {
		t.Errorf("expected baseline state %q, got %q", types.StateUnchanged, result.BaselineDiffs[0].State)
	}
}
...
func TestDiffBaselines_SV001Updated(t *testing.T) {
	...
	if req.State != types.StateUpdated {
		t.Errorf("expected SV-001 state %q, got %q", types.StateUpdated, req.State)
	}
}
...
func TestDiffBaselines_SV002Unchanged(t *testing.T) {
	...
	if req.State != types.StateUnchanged {
		t.Errorf("expected SV-002 state %q, got %q", types.StateUnchanged, req.State)
	}
}
...
func TestDiffBaselines_AbsentAndNew(t *testing.T) {
	...
	tests := []struct {
		id            string
		expectedState types.RequirementState
		beforeNil     bool
		afterNil      bool
	}{
		{"SV-003", types.StateAbsent, false, true},
		{"SV-004", types.StateNew, true, false},
	}
...
func TestDiffBaselines_ChangeReasons(t *testing.T) {
	...
	for _, r := range req.ChangeReasons {
		if r == types.ReasonImpactChanged {
			hasImpactChanged = true
		}
		if r == types.ReasonMetadataChanged {
