...
func findComponentDiff(diffs []types.ComponentDiff, name string) *types.ComponentDiff {
	for i := range diffs {
...
func TestDiffSystems_ComparisonMode(t *testing.T) {
	...
	if result.ComparisonMode != types.ModeSystemDrift {
		t.Errorf("expected comparisonMode %q, got %q", types.ModeSystemDrift, result.ComparisonMode)
	}
}
...
func TestDiffSystems_Sources(t *testing.T) {
	...
	if result.Sources[0].Role != types.RoleOld {
		t.Errorf("expected source[0] role %q, got %q", types.RoleOld, result.Sources[0].Role)
	}
	if result.Sources[1].Role != types.RoleNew {
		t.Errorf("expected source[1] role %q, got %q", types.RoleNew, result.Sources[1].Role)
	}
...
func TestDiffSystems_WebTierUpdated(t *testing.T) {
	...
	if comp.State != types.StateUpdated {
		t.Errorf("expected WebTier state %q, got %q", types.StateUpdated, comp.State)
	}
}
...
func TestDiffSystems_DatabaseTierUnchanged(t *testing.T) {
	...
	if comp.State != types.StateUnchanged {
		t.Errorf("expected DatabaseTier state %q, got %q", types.StateUnchanged, comp.State)
	}
}
...
func TestDiffSystems_LegacyAPIAbsent(t *testing.T) {
	...
	if comp.State != types.StateAbsent {
		t.Errorf("expected LegacyAPI state %q, got %q", types.StateAbsent, comp.State)
	}
	if comp.Before == nil {
...
func TestDiffSystems_CacheTierNew(t *testing.T) {
	...
	if comp.State != types.StateNew {
		t.Errorf("expected CacheTier state %q, got %q", types.StateNew, comp.State)
	}
	if comp.Before != nil {
...
func TestDiffSystems_AuthorizationStatusChange(t *testing.T) {
	...
	sysChanges, ok := result.Extensions["systemFieldChanges"].([]types.FieldChange)
	if !ok {
	...
	for _, fc := range sysChanges {
		if fc.Path == "authorizationStatus" {
			found = true
			if fc.Op != types.OpReplace {
				t.Errorf("expected op %q, got %q", types.OpReplace, fc.Op)
			}
