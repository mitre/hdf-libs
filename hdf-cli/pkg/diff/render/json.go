...
type jsonSummary struct {
	...
	ComparisonMode types.ComparisonMode    `json:"comparisonMode"`
	Summary        types.ComparisonSummary `json:"summary"`
}

// strippedRequirementDiff is a RequirementDiff without Before/After snapshots.
type strippedRequirementDiff struct {
	ID                 string                 `json:"id"`
	State              types.RequirementState `json:"state"`
	ChangeReasons      []types.ChangeReason   `json:"changeReasons"`
	...
	FieldChanges       []types.FieldChange    `json:"fieldChanges"`
...
type strippedComparison struct {
	...
	Matching         *types.MatchingConfig       `json:"matching,omitempty"`
	Summary          types.ComparisonSummary     `json:"summary"`
	BaselineDiffs    []types.BaselineDiff        `json:"baselineDiffs"`
...
func stripDiff(d types.RequirementDiff) strippedRequirementDiff {
...
func JSON(comparison types.HdfComparison, opts Options) (string, error) {
