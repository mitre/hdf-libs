...
func Render(comparison types.HdfComparison, format string, opts Options) (string, error) {
	switch format {
	case "json":
...
func filterRequirements(diffs []types.RequirementDiff, opts Options) []types.RequirementDiff {
	...
	var filtered []types.RequirementDiff
	for _, d := range diffs {
		if allowed[d.State] {
...
func formatFieldChangesWithArrow(changes []types.FieldChange, arrow string) string {
	if len(changes) == 0 {
		return ""
	}
