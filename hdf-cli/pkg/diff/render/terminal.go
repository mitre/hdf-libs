...
func Terminal(comparison types.HdfComparison, opts Options) (string, error) {
	...
	for _, req := range filtered {
		...
		if detail == DetailControl && req.State == types.StateUnchanged {
			continue
		}
...
func symbolAndColor(state types.RequirementState, useColor bool) (string, func(string) string) {
	identity := func(s string) string { return s }

	switch state {
	case types.StateFixed, types.StateNew:
		if useColor {
			return "+", func(s string) string { return ansiGreen + s + ansiReset }
		}
		return "+", identity
	case types.StateRegressed, types.StateAbsent:
		if useColor {
	...
	case types.StateUpdated:
		...
		return "~", identity
	case types.StateUnchanged, types.StateMoved, types.StateSplit, types.StateMerged:
		if useColor {
...
func renderRequirementLine(req types.RequirementDiff, detail DetailLevel, useColor bool) string {
...
func formatStatusTransition(req types.RequirementDiff) string {
...
func buildHeaderLine(comparison types.HdfComparison, useColor bool) string {
	...
	for _, s := range comparison.Sources {
		switch s.Role {
		case types.RoleOld, types.RoleGolden, types.RoleReference:
			if oldTimestamp == "" {
				oldTimestamp = s.AssessmentTimestamp
			}
		case types.RoleNew, types.RoleSystem:
			if newTimestamp == "" {
...
func buildSummaryLine(comparison types.HdfComparison, useColor bool) string {
