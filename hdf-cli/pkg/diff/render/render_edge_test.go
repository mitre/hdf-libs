...
func fieldChangeFixture() types.HdfComparison {
		...
		RequirementDiffs: []types.RequirementDiff{
			{
				...
				ChangeReasons:      []types.ChangeReason{types.ReasonMetadataChanged},
				Before:             &hdf.EvaluatedRequirement{},
				After:              &hdf.EvaluatedRequirement{},
				FieldChanges: []types.FieldChange{
					{Op: types.OpAdd, Path: "tags.newTag", NewValue: "added-val"},
					{Op: types.OpRemove, Path: "tags.oldTag", OldValue: "removed-val"},
					{Op: types.OpReplace, Path: "impact", OldValue: 0.3, NewValue: 0.7},
				},
			},
		},
...
func TestJSON_FilterStates_NonExistentState(t *testing.T) {
	...
	out, err := JSON(comp, Options{
		Detail:       DetailControl,
		FilterStates: []types.RequirementState{"critical"},
	})
	require.NoError(t, err)
...
func TestJSON_FullDetail_FilterStatesPreservesBeforeAfter(t *testing.T) {
	...
	out, err := JSON(comp, Options{
		Detail:       DetailFull,
		FilterStates: []types.RequirementState{types.StateRegressed},
	})
	require.NoError(t, err)
...
func TestMarkdown_EmptyStateSectionsShowNone(t *testing.T) {
	...
	comp := testFixture()
	comp.RequirementDiffs = []types.RequirementDiff{comp.RequirementDiffs[0]}
...
func emptyTitleReq(id string, state types.RequirementState, oldStatus, newStatus string) types.HdfComparison {
	comp := testFixture()
...
func TestMarkdown_EmptyTitle_FullDetail(t *testing.T) {
	comp := emptyTitleReq("V-3001", types.StateFixed, "failed", "passed")
	out, err := Markdown(comp, Options{Detail: DetailFull})
	require.NoError(t, err)
...
func TestMarkdown_EmptyTitle_ControlDetail(t *testing.T) {
	comp := emptyTitleReq("V-3002", types.StateRegressed, "passed", "failed")
	out, err := Markdown(comp, Options{Detail: DetailControl})
	require.NoError(t, err)
...
func TestTerminal_HandlesMissingTitleAndStatuses(t *testing.T) {
	comp := types.HdfComparison{
		...
		ComparisonMode: types.ModeTemporal,
		Sources:        []types.Source{},
		Summary:        types.ComparisonSummary{Updated: 1, Total: 1},
		RequirementDiffs: []types.RequirementDiff{
			{
				ID:            "V-5001",
				State:         types.StateUpdated,
				...
				FieldChanges: []types.FieldChange{
					{Op: types.OpAdd, Path: "tags.newTag", NewValue: "val"},
					{Op: types.OpRemove, Path: "tags.oldTag", OldValue: "val"},
				},
			},
		},
		BaselineDiffs: []types.BaselineDiff{},
...
func TestTerminal_GoldenRoleForBaselineMode(t *testing.T) {
	comp := testFixture()
	comp.ComparisonMode = types.ModeBaseline
	comp.Sources = []types.Source{
		{Role: types.RoleGolden, Label: "Golden baseline", AssessmentTimestamp: "2024-01-01T00:00:00Z"},
		{Role: types.RoleNew, Label: "Current scan", AssessmentTimestamp: "2024-02-01T00:00:00Z"},
	}
	out, err := Terminal(comp, Options{Detail: DetailSummary, NoColor: true})
	require.NoError(t, err)
...
func TestTerminal_ReferenceSystemRolesForFleetMode(t *testing.T) {
	comp := testFixture()
	comp.ComparisonMode = types.ModeFleet
	comp.Sources = []types.Source{
		{Role: types.RoleReference, Label: "Reference", AssessmentTimestamp: "2024-01-01T00:00:00Z"},
		{Role: types.RoleSystem, Label: "System 1", AssessmentTimestamp: "2024-02-01T00:00:00Z"},
	}
	out, err := Terminal(comp, Options{Detail: DetailSummary, NoColor: true})
	require.NoError(t, err)
...
func csvTitleEscapeFixture(id string, state types.RequirementState, title string) types.HdfComparison {
	comp := testFixture()
	comp.RequirementDiffs = []types.RequirementDiff{
		{
			ID:            id,
...
func TestCSV_DoubleQuoteEscaping(t *testing.T) {
	comp := csvTitleEscapeFixture("V-6001", types.StateFixed, `Ensure "root" login disabled`)
	out, err := CSV(comp, Options{Detail: DetailControl})
	require.NoError(t, err)
...
func TestCSV_NewlineEscaping(t *testing.T) {
	comp := csvTitleEscapeFixture("V-6002", types.StateNew, "Line one\nLine two")
	out, err := CSV(comp, Options{Detail: DetailControl})
	require.NoError(t, err)
...
func TestCSV_FilterStates_MultipleStates(t *testing.T) {
	...
	out, err := CSV(comp, Options{
		Detail:       DetailControl,
		FilterStates: []types.RequirementState{types.StateFixed, types.StateNew},
	})
	require.NoError(t, err)
...
func TestFilterRequirements_EmptyStatesArrayReturnsAll(t *testing.T) {
	...
	filtered := filterRequirements(comp.RequirementDiffs, Options{
		FilterStates: []types.RequirementState{},
	})
...
func TestFilterRequirements_NoMatchReturnsEmpty(t *testing.T) {
	comp := testFixture()
	filtered := filterRequirements(comp.RequirementDiffs, Options{
		FilterStates: []types.RequirementState{"nonexistent"},
	})
...
func TestFilterRequirements_EmptyDiffsReturnsEmpty(t *testing.T) {
	filtered := filterRequirements([]types.RequirementDiff{}, Options{
		FilterStates: []types.RequirementState{types.StateFixed},
	})
	assert.Empty(t, filtered)
}
...
func TestFormatFieldChanges_AddOp(t *testing.T) {
	changes := []types.FieldChange{
		{Op: types.OpAdd, Path: "tags.newTag", NewValue: "added"},
	}
...
func TestFormatFieldChanges_RemoveOp(t *testing.T) {
	changes := []types.FieldChange{
		{Op: types.OpRemove, Path: "tags.oldTag", OldValue: "removed"},
	}
	result := formatFieldChangesWithArrow(changes, "->")
...
func TestFormatFieldChanges_MultipleJoinedWithSemicolon(t *testing.T) {
	changes := []types.FieldChange{
		{Op: types.OpAdd, Path: "a", NewValue: "x"},
		{Op: types.OpRemove, Path: "b", OldValue: "y"},
	}
	result := formatFieldChangesWithArrow(changes, "->")
	assert.Contains(t, result, "; ")
}
...
func TestFormatFieldChanges_NilValues(t *testing.T) {
	changes := []types.FieldChange{
		{Op: types.OpAdd, Path: "tags.new", NewValue: nil},
	}
	result := formatFieldChangesWithArrow(changes, "->")
...
func TestFormatFieldChanges_UnicodeArrow(t *testing.T) {
	changes := []types.FieldChange{
		{Op: types.OpReplace, Path: "title", OldValue: "old", NewValue: "new"},
	}
	result := formatFieldChangesWithArrow(changes, "\u2192")
	assert.Contains(t, result, "\u2192")
}
...
func TestSymbolAndColor_FixedGreen(t *testing.T) {
	sym, colorFn := symbolAndColor(types.StateFixed, true)
	assert.Equal(t, "+", sym)
...
func TestSymbolAndColor_RegressedRed(t *testing.T) {
	sym, colorFn := symbolAndColor(types.StateRegressed, true)
	assert.Equal(t, "-", sym)
	colored := colorFn("test")
...
func TestSymbolAndColor_NewGreen(t *testing.T) {
	sym, _ := symbolAndColor(types.StateNew, true)
	assert.Equal(t, "+", sym)
}

func TestSymbolAndColor_AbsentRed(t *testing.T) {
	sym, _ := symbolAndColor(types.StateAbsent, true)
	assert.Equal(t, "-", sym)
}

func TestSymbolAndColor_UpdatedYellow(t *testing.T) {
	sym, colorFn := symbolAndColor(types.StateUpdated, true)
	assert.Equal(t, "~", sym)
	colored := colorFn("test")
...
func TestSymbolAndColor_UnchangedDim(t *testing.T) {
	sym, colorFn := symbolAndColor(types.StateUnchanged, true)
	assert.Equal(t, " ", sym)
	colored := colorFn("test")
...
func TestSymbolAndColor_NoColorIdentity(t *testing.T) {
	_, colorFn := symbolAndColor(types.StateFixed, false)
...
func TestFormatStatusTransition_NoStatuses(t *testing.T) {
	req := types.RequirementDiff{State: types.StateUpdated}
	result := formatStatusTransition(req)
