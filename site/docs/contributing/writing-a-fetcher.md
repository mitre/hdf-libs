...
Every Go fetcher offers two ways to build a client, and the difference is who owns *transport*. The parameters differ by service — the HTTP-based fetchers take TLS options and build a stdlib client, while the AWS ones take a context and build an SDK client — but the pairing is constant.
...
// library never touches transport or TLS.
...
Reach for the injection form whenever the application layer needs to own transport — corporate proxies, TLS interception, or a mocked client in tests.

Injection does **not** hand over credentials, and that catches people out. An HTTP fetcher resolves its own token inside `Fetch`, from environment variables and, where the tool has one, its CLI's config file: `GITLAB_TOKEN` / `GLAB_TOKEN` / the glab config for `gitlab` and `gitlab-vulnerabilities`, `DEFECTDOJO_API_TOKEN`, `SONARQUBE_TOKEN`, `SPLUNK_TOKEN`. Both constructors do this. The library never accepts a token as a parameter, never logs one and never lets one into an error message. Only the AWS fetchers differ, because there the injected SDK client carries the credentials itself.

That split is deliberate: Go is what `hdf-cli` drives, and resolving the token in the library keeps it out of the CLI's process and out of `ps`. The auth-agnostic TypeScript rule below is the other half of the same decision.
...
Go is required, because `hdf-cli` drives it; TypeScript is added when a JavaScript consumer needs it, and most fetchers in tree are Go-only.
...
- **Never truncate a nested list silently.** A page of findings often carries its own connections — a triage history, a list of overrides. Those paginate too, and a fixed `first:` on them returns whatever end the API orders from, which is usually the oldest while the consumer reads the newest as current. Ask for the page-info alongside the nodes, then either complete the list with a follow-up request or fail; `gitlab-vulnerabilities` does the former.
- **Do not follow redirects with a credential attached.** A 3xx re-sends the token to whatever host it names. Refuse redirects outright where the API never legitimately issues one, as `gitlab-vulnerabilities` does; where one is expected — the `gitlab` artifact fetcher follows GitLab's redirect to object storage — the credential header has to be stripped when the host changes.
...
## What the API returns is not always what it means

Two failure modes look like success and are worth designing against before writing the first request.

**Request everything the converter maps.** A narrow field selection silently strips data the converter would have translated, and nothing fails — the output is simply poorer than the source. Ask for the expansions, nested objects and union branches the converter has code for, and re-check the request whenever the converter grows a branch.

**An empty result is not always an empty report.** On a tiered product, or one that ingests asynchronously, the API answers a query about a feature you do not have with an empty list rather than an error, and a fetcher that trusts it emits a document that reads as a clean bill of health. Find the signal that ingestion itself leaves behind, judge the empty case against that, and refuse to write a document when the answer is "this was never populated" — naming which of the reasons it was. `gitlab-vulnerabilities` is the worked example: its CI-derived scanner fields read identically on every tier, so it uses the statistics row and scan statuses instead. The same trap applies to a filtered query: record the filter in what you hand the converter, or a selection that matched nothing is indistinguishable from a source with nothing to report.

## Adding one

1. Pick a tool name in `kebab-case` and create `fetchers/<tool>/{go,typescript}/`.
2. Implement Go with the two constructors. Add the TypeScript twin, auth-agnostic, only when a JavaScript consumer needs it.
