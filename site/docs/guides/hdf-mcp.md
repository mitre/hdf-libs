...
Rules: pass exactly one of `source` / `sources`; a one-element `sources` is the
single-source call, unchanged; every member must be a valid **results**
document (a baseline document can be queried alone, not in a set), and a member
that fails to load, is another type, or is schema-invalid refuses the whole set
naming its index (`sources[1] is a system document …`) — a set is never
partially answered. If two baselines still share a name after the tool prefix,
`notice` says so; rows and groups are keyed by position, so the answer is
unaffected.
