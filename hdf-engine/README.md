...
Shared, schema-typed read-side engines for Heimdall Data Format (HDF) documents — document-type **detection**, requirement **query/filtering**, **compliance** rollups, evidence-package **verification**, and multi-document **merge** (one baseline per scanner, per ADR-0016), with more read-side engines to follow.
