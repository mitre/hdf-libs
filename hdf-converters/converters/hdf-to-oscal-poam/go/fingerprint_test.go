...
func TestHdfToOscalPoamFingerprint(t *testing.T) {
	t.Run("is registered with correct metadata", func(t *testing.T) {
		fp := registry.GetFingerprint("hdf-to-oscal-poam")
		require.NotNil(t, fp, "hdf-to-oscal-poam should be registered via init()")
		assert.Equal(t, "HDF to OSCAL POA&M", fp.Label)
