...
func TestConvert_EmptyInput(t *testing.T) {
	_, err := ConvertMsftDefenderEndpointToHDF([]byte(""), testVersion)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "empty input")
}

func TestConvert_InvalidJSON(t *testing.T) {
	_, err := ConvertMsftDefenderEndpointToHDF([]byte("not json"), testVersion)
	assert.Error(t, err)
...
func TestConvert_DataSource(t *testing.T) {
	...
	assert.Equal(t, "Microsoft Defender for Endpoint", *result.DataSource.Name)
}
