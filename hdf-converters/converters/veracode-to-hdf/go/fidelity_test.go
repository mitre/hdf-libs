...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertVeracodeToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "Veracode CWE categories plus distinct SCA CVE ids", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`<summaryreport/>`), []byte(`<detailedreport/>`)} {
		_, convErr := ConvertVeracodeToHDF(bad, "test")
