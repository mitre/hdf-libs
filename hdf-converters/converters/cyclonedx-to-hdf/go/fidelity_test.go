...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertCycloneDXToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
// One requirement per vulnerabilities[] entry within the size limit; a BOM with no vulnerabilities is rejected, never converted to a placeholder.
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "CycloneDX vulnerabilities", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("{}"), []byte("garbage"), []byte("{\"bomFormat\":\"CycloneDX\"}"), []byte("{\"bomFormat\":\"CycloneDX\",\"components\":[{\"name\":\"a\"}]}"), []byte("{\"bomFormat\":\"CycloneDX\",\"vulnerabilities\":[{\"id\":\"CVE-2021-44228\"}]}")} {
