...
describe('gosec to HDF converter', () => {
  describe('convertGosecToHdf', () => {
    ...
    it('should produce valid HDF structure from Go Ethereum fixture', () => {
      const output = convertGosecToHdf(loadFixture('ethereum.json'));
    ...
    it('should use "gosec Scan" as the baseline name', () => {
      const hdf = JSON.parse(convertGosecToHdf(loadFixture('ethereum.json'))) as HdfResults;
    ...
    it('should mark externally suppressed issues as notReviewed', () => {
      const hdf = JSON.parse(convertGosecToHdf(loadFixture('ethereum.json'))) as HdfResults;
    ...
    it('should include code_desc with rule, file, and line', () => {
      const hdf = JSON.parse(convertGosecToHdf(loadFixture('ethereum.json'))) as HdfResults;
      const g304 = hdf.baselines[0]!.requirements.find(r => r.id === 'G304');
