...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertAWSConfigToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
// One requirement per ConfigRules[] entry within the size limit; an empty export yields none (the converter carries no no-findings placeholder).
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "AWS Config rules", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("{}"), []byte("[]"), []byte("garbage"), []byte("{\"ConfigRules\":[]}"), []byte("{\"ConfigRules\":null}")} {
