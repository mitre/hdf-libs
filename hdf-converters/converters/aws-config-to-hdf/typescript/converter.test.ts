...
describe('AWS Config to HDF converter', () => {
  describe('convertAwsConfigToHdf', () => {
    it('should throw on invalid JSON', () => {
      expect(() => convertAwsConfigToHdf('not json')).toThrow();
    });

    it('should throw on empty input', () => {
      expect(() => convertAwsConfigToHdf('')).toThrow();
    });

    it('should throw when ConfigRules field is missing', () => {
      expect(() => convertAwsConfigToHdf(JSON.stringify({ other: 'field' }))).toThrow(
        'ConfigRules field is required'
      );
    });

    it('should produce valid HDF structure from minimal fixture', () => {
      const output = convertAwsConfigToHdf(loadFixture('minimal.json'));
    ...
    it('should use "AWS Config" as the baseline name', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      expect(hdf.baselines[0]!.name).toBe('AWS Config');
    });

    it('should set baseline title and maintainer', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
    ...
    it('should create one requirement per config rule', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      expect(hdf.baselines[0]!.requirements).toHaveLength(1);
    });

    it('should use ConfigRuleId as the requirement ID', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      expect(hdf.baselines[0]!.requirements[0]!.id).toBe('config-rule-7hytm9');
    });

    it('should set impact to 0.5 for all rules', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      expect(hdf.baselines[0]!.requirements[0]!.impact).toBe(0.5);
    });

    it('should include title with account ID and rule name', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
    ...
    it('should include default description from rule Description field', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
    ...
    it('should include check description with ARN and source identifier', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      const req = hdf.baselines[0]!.requirements[0]!;
    ...
    it('should include check description with input parameters', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      const req = hdf.baselines[0]!.requirements[0]!;
      const checkDesc = req.descriptions?.find(d => d.label === 'check');
    ...
    it('should include source location with ARN', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      const req = hdf.baselines[0]!.requirements[0]!;
    ...
    it('should look up NIST tags via source identifier', () => {
    ...
    it('should map COMPLIANT to passed', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
    ...
    it('should map NON_COMPLIANT to failed', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      const results = hdf.baselines[0]!.requirements[0]!.results;
    ...
    it('should include code_desc with rule name, resource type, resource id', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
    ...
    it('should include failure message only for failed results', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('minimal.json'))) as HdfResults;
      const results = hdf.baselines[0]!.requirements[0]!.results;
    ...
    it('should handle multiple rules from multi-rule fixture', () => {
      const hdf = JSON.parse(convertAwsConfigToHdf(loadFixture('multi-rule.json'))) as HdfResults;
      expect(hdf.baselines[0]!.requirements).toHaveLength(4);
    });

    it('should map NOT_APPLICABLE to notApplicable', () => {
      ...
      const hdf = JSON.parse(convertAwsConfigToHdf(input)) as HdfResults;
      expect(hdf.baselines[0]!.requirements[0]!.results[0]!.status).toBe('notApplicable');
    });

    it('should map INSUFFICIENT_DATA to notReviewed', () => {
      const input = JSON.stringify({
        ConfigRules: [{
          ConfigRuleId: 'config-rule-test',
          ConfigRuleName: 'test-rule',
    ...
    it('should handle rule with no NIST mapping gracefully', () => {
      const input = JSON.stringify({
        ConfigRules: [{
    ...
    it('should handle empty EvaluationResults', () => {
      const input = JSON.stringify({
        ConfigRules: [{
