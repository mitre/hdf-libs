...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertSplunkToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "Splunk control events attached to a profile event", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`[]`), []byte(`[{"meta":{"guid":"g","subtype":"control"}}]`)} {
		_, convErr := ConvertSplunkToHDF(bad, "test")
