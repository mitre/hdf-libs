...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertDefectDojo(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
// One requirement per finding whether the input is the DRF envelope or a bare array, with no size limit, or one no-findings requirement when there are none.
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "DefectDojo findings", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("{}"), []byte("[]"), []byte("garbage"), []byte("{\"results\":[]}"), []byte("{\"results\":null}")} {
