...
func countOverrides(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertCSAFVEXToHDF(data, "test")
	if err != nil {
		return 0, err
	}
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "CSAF VEX actionable product-status buckets", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`{"document":{"category":"csaf_vex"},"vulnerabilities":[]}`)} {
		_, convErr := ConvertCSAFVEXToHDF(bad, "test")
...
func TestExpectedRequirementCount_ZeroOverridesIsAnError(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "fixtures", "input", "2022-evd-uc-01-a-001.json"))
	require.NoError(t, err)
	_, convErr := ConvertCSAFVEXToHDF(data, "test")
