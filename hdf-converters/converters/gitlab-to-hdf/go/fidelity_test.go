...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertGitlabToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
// One requirement per vulnerabilities[] entry within the size limit, or one no-findings requirement when the array is empty or absent.
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("{}"), []byte("garbage"), []byte("[]"), []byte("{\"vulnerabilities\":[]}"), []byte("{\"vulnerabilities\":null}")} {
