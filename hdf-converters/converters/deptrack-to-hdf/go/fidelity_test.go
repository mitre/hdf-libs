...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertDeptrackToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
// One requirement per findings[] entry within the size limit, or one no-findings requirement when there are none.
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "Dependency-Track findings", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("{}"), []byte("garbage"), []byte("{\"findings\":[]}"), []byte("{\"project\":{\"uuid\":\"u\"}}")} {
