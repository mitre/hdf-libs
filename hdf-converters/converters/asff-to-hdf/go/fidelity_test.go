...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertAsffToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "distinct ASFF control ids per product baseline", unit)
...
func TestExpectedRequirementCount_Vectors(t *testing.T) {
	cases := []struct {
		...
		{"trivy_sample.json", 23},
		{"prowler_sample.json", 2},
	}
	for _, c := range cases {
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}")} {
		_, convErr := ConvertAsffToHDF(bad, "test")
