...
func countOverrides(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertSPDXVEXToHDF(data, "test")
	if err != nil {
		return 0, err
	}
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "SPDX VEX relationships resolving to a CVE", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`{"@graph":[]}`)} {
		_, convErr := ConvertSPDXVEXToHDF(bad, "test")
...
func TestExpectedRequirementCount_ZeroOverridesIsAnError(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "fixtures", "input", "no-actionable.spdx.json"))
	require.NoError(t, err)
	_, convErr := ConvertSPDXVEXToHDF(data, "test")
