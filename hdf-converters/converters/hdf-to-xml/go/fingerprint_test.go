...
func TestHdfToXmlFingerprint(t *testing.T) {
	t.Run("is registered with correct metadata", func(t *testing.T) {
		fp := registry.GetFingerprint("hdf-to-xml")
		require.NotNil(t, fp, "hdf-to-xml should be registered via init()")
		assert.Equal(t, "HDF to XML", fp.Label)
