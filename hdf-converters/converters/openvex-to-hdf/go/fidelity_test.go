...
func countOverrides(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertOpenVEXToHDF(data, "test")
	if err != nil {
		return 0, err
	}
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	...
	for _, path := range inputs {
		...
		require.Equal(t, "OpenVEX statements with an actionable status", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`{"statements":[]}`)} {
		_, convErr := ConvertOpenVEXToHDF(bad, "test")
...
func TestExpectedRequirementCount_ZeroOverridesIsAnError(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "fixtures", "input", "empty.openvex.json"))
	require.NoError(t, err)
	_, convErr := ConvertOpenVEXToHDF(data, "test")
