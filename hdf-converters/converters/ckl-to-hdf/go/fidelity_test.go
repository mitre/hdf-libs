...
func countRequirements(t *testing.T, data []byte) (int, error) {
	t.Helper()
	result, err := ConvertCKLToHDF(data, "test")
	if err != nil {
		return 0, err
	}
	n := 0
...
func TestExpectedRequirementCount_MatchesConversionForEveryFixture(t *testing.T) {
	inputs, err := filepath.Glob(filepath.Join("..", "fixtures", "input", "*.ckl"))
	...
	for _, path := range inputs {
		...
		require.Equal(t, "CKL VULN entries", unit)
...
func TestExpectedRequirementCount_RejectsWhatTheConverterRejects(t *testing.T) {
	for _, bad := range [][]byte{nil, []byte(""), []byte("garbage"), []byte("{}"), []byte(`<CHECKLIST><STIGS></STIGS></CHECKLIST>`), []byte(`<CHECKLIST><STIGS><iSTIG></iSTIG></STIGS></CHECKLIST>`)} {
		_, convErr := ConvertCKLToHDF(bad, "test")
