// Package convert is the importable, cobra/console-free converter registry for
// the HDF ecosystem. It holds the format-pair map, the Register/Get/List API,
// the wrapper types + register helpers, and self-registers every converter via
// init() in this package. Both the CLI (package cmd) and the MCP
// (hdf-cli/internal/mcp) import it and share one populated registry.
//
// It is a sibling to registry/all (the fingerprint aggregator): both are
// downstream of the converter packages, importing them to trigger init(). It
// must NOT live in package registry itself — converters import that package for
// fingerprints, so a converter-importing registry there would be an import
// cycle.
//
// Keeping this package free of cobra and any console/output dependency is a
// hard requirement: the stdio MCP process imports it, and stray stdout or a
// cobra dependency would corrupt the protocol stream (ADR-0007 §14).
package convert
...
// version is the generator version stamped into converter output. Adapters
// (the CLI, the MCP) inject it at startup via SetVersion; it defaults to "dev".
// Replaces the former package cmd `version` global the wrappers used to close
// over, so the registry no longer depends on cmd-internal state.
var version = "dev"

// SetVersion sets the generator version the converter wrappers stamp into HDF
// output. Call it once at process startup from each adapter.
...
// oscal-profile conversions. The CLI binds its --catalog flag to it via
// SetOSCALCatalogPath before invoking the profile converter; the MCP would set
// it from a tool argument. It replaces the former package cmd `oscalCatalogFlag`
// global the profile converter read directly.
var oscalCatalogPath string

// SetOSCALCatalogPath sets the catalog path the oscal-profile converter reads.
func SetOSCALCatalogPath(p string) { oscalCatalogPath = p }

// HandlesOutputVersionInternally reports whether c performs HDF output-version
// transforms itself (the hdf→hdf version converter), so callers should skip
// external version post-processing for it. Kept as a predicate so the concrete
// converter type stays unexported.
func HandlesOutputVersionInternally(c Converter) bool {
	_, ok := c.(*hdfVersionConverter)
	return ok
}
