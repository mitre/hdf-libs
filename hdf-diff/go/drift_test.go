...
func findDrift(drift []RequirementDiff, id string) *RequirementDiff {
	for i := range drift {
		if drift[i].ID == id {
			return &drift[i]
		}
...
func containsReason(reasons []ChangeReason, r ChangeReason) bool {
...
func TestDrift_ArrayAlwaysPresent(t *testing.T) {
	...
	if comp.Drift == nil {
		...
		comp.Drift = []RequirementDiff{} // normalize for clarity
	}
	if len(comp.Drift) != 0 {
...
func TestDrift_NoDriftWhenIdentical(t *testing.T) {
	...
	if comp.RequirementDiffs[0].State != StateUnchanged {
...
func TestDrift_TagsChangedStatusSame(t *testing.T) {
	...
	if !containsReason(req.ChangeReasons, ReasonMetadataChanged) {
	...
	if !containsReason(driftReq.ChangeReasons, ReasonMetadataChanged) {
...
func TestDrift_SingleMetadataChange(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedReason: ReasonImpactChanged,
		},
		{
			...
			expectedReason: ReasonMetadataChanged,
		},
	}
...
func TestDrift_DescriptionChanged(t *testing.T) {
	...
	assertSingleDriftWithReason(t, comp, reqIDSV001, ReasonMetadataChanged)
}
...
func TestDrift_FieldChangesPopulated_Impact(t *testing.T) {
	...
	for _, fc := range driftReq.FieldChanges {
		if fc.Op == OpReplace && fc.Path == fieldImpact {
...
func TestDrift_FieldChangesPopulated_Tags(t *testing.T) {
	...
	found := false
	for _, fc := range driftReq.FieldChanges {
		if fc.Op == OpReplace && fc.Path == "tags" {
			found = true
		}
	}
	if !found {
...
func TestDrift_ChangeReasonsPopulated(t *testing.T) {
	...
	if !containsReason(driftReq.ChangeReasons, ReasonImpactChanged) {
