...
func TestBaseline_ComparisonModeIsBaseline(t *testing.T) {
	...
	result := mustDiffHdf(t, goldenDoc, []hdf.HDFResults{currentDoc}, baselineOpts())

	if result.ComparisonMode != ModeBaseline {
		t.Errorf("expected comparisonMode 'baseline', got %q", result.ComparisonMode)
	}
}
...
// ---------------------------------------------------------------------------
// 2. result.Sources with correct roles and labels
// ---------------------------------------------------------------------------
...
func TestBaseline_FirstSourceRoleIsGolden(t *testing.T) {
	...
	if result.Sources[0].Role != RoleGolden {
		t.Errorf("expected first source role 'golden', got %q", result.Sources[0].Role)
	}
}
...
func TestBaseline_FirstSourceLabelIsGoldenBaseline(t *testing.T) {
	...
	if result.Sources[0].Label != "Golden baseline" {
		t.Errorf("expected first source label 'Golden baseline', got %q", result.Sources[0].Label)
	}
}
...
func TestBaseline_SecondSourceLabelIsCurrentScan(t *testing.T) {
	...
	if result.Sources[1].Label != "Current scan" {
		t.Errorf("expected second source label 'Current scan', got %q", result.Sources[1].Label)
	}
}
...
func TestBaseline_SV003MarkedAsRegressed(t *testing.T) {
	...
	req := findReq(result.RequirementDiffs, "SV-003")
	if req == nil {
...
func TestBaseline_BaselineVersionDiffDetected(t *testing.T) {
	...
	bd := findBaselineDiff(result.BaselineDiffs, "rhel9-stig-baseline")
	if bd == nil {
...
func TestBaseline_IdenticalDocuments_AllUnchanged(t *testing.T) {
	goldenDoc := buildBaselineScanBefore()
	result := mustDiffHdf(t, goldenDoc, []hdf.HDFResults{goldenDoc}, baselineOpts())
