...
func TestUbuntu_FormatVersionAndComparisonMode(t *testing.T) {
	...
	if ubuntu.diff.ComparisonMode != ModeTemporal {
		t.Errorf("comparisonMode = %q, want %q", ubuntu.diff.ComparisonMode, ModeTemporal)
	}
}
...
func TestUbuntu_FixedHaveResultChanged(t *testing.T) {
	...
	fixedReqs := filterByState(ubuntu.diff.RequirementDiffs, StateFixed)
	if len(fixedReqs) == 0 {
