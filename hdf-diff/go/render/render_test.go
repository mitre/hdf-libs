...
func testFixture() diff.HdfComparison {
	oldImpact := 0.7
	newImpact := 0.5
	...
	return diff.HdfComparison{
		...
		RequirementDiffs: []diff.RequirementDiff{
			...
			},
			{
				...
				NewEffectiveStatus: "passed",
				ChangeReasons:      []diff.ChangeReason{diff.ReasonControlMapped},
				After:              &hdf.EvaluatedRequirement{},
				FieldChanges:       []diff.FieldChange{},
			},
			{
				ID:                 "V-1004",
				State:              diff.StateAbsent,
				Title:              "Removed Check",
				OldEffectiveStatus: "passed",
			...
			},
			{
				...
				After:              &hdf.EvaluatedRequirement{},
				FieldChanges: []diff.FieldChange{
					{Op: diff.OpReplace, Path: "/impact", OldValue: 0.7, NewValue: 0.5},
				},
			},
		},
...
func TestJSON_FilterStates(t *testing.T) {
	...
	out, err := JSON(comp, Options{
		Detail:       DetailFull,
		FilterStates: []diff.RequirementState{diff.StateFixed, diff.StateRegressed},
	})
	require.NoError(t, err)
...
func TestCSV_ProperEscaping(t *testing.T) {
	...
	comp.RequirementDiffs = append(comp.RequirementDiffs, diff.RequirementDiff{
		ID:            "V-1007",
		State:         diff.StateNew,
...
func TestFilterRequirements_SingleState(t *testing.T) {
	...
	assert.Equal(t, diff.StateFixed, filtered[0].State)
}

func TestFilterRequirements_MultipleStates(t *testing.T) {
	comp := testFixture()
	filtered := filterRequirements(comp.RequirementDiffs, Options{
		FilterStates: []diff.RequirementState{diff.StateFixed, diff.StateRegressed, diff.StateNew},
	})
