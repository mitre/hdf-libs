...
- A proposed small optional **derivation block** on `hdf-results` records the lineage: the seed snapshot as a **`Content_Reference`** (`{uri, checksum}` — identity + locator + integrity, the prevailing location-plus-expected-hash pattern of SRI / OCI descriptors / SPDX external document refs, and already our house shape), the producer `source`, `throughSequence` (event watermark), `eventsApplied`, and `asOf`. Conceptually this is W3C PROV's qualified derivation (derived entity ← used entity + activity + generation time) / OpenLineage's dataset-version facet. **This is a schema addition and is flagged per the field-gap discipline**: generalized as *derived-document lineage* (any materialized/derived document could use it), optional, to be designed in the schema phase — not silently added.
...
4. **Phase 4a — In-process reference reconciler (external; the primary hands-on validation).** A broker-free tool (scratch project / SAF) that reads an event sequence from files or stdin, holds the keyed last-value map in memory, calls the three kernel functions, and writes the resulting comparison + reconciled result set. Zero infrastructure. It dogfoods the hdf-libs API end-to-end and surfaces ergonomics problems before any runtime exists. *Note:* external to hdf-libs — it validates the library API but is not a repo deliverable. If the kernel cannot cleanly power this dead-simple reconciler, fix the kernel first.
