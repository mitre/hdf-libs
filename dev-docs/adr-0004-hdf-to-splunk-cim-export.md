...
4. **Ship a minimal companion TA (`Splunk_TA_hdf`).** In-repo, a small Splunk add-on: `props.conf` (sourcetype recognition, JSON `KV_MODE`, `FIELDALIAS`/`EVAL` mapping our flat scalars to CIM names, and `TIME_*`/epoch handling), `eventtypes.conf` (`[hdf_finding] search = sourcetype=hdf:results (hdf_status=failed OR hdf_status=error OR cvss>0)`), and `tags.conf` (`[eventtype=hdf_finding] vulnerability=enabled report=enabled`). This is the piece that makes the output genuinely CIM-live; without it the JSON is CIM-shaped but populates no data model.
