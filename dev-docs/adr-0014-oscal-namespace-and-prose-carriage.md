...
- **Revision:** 2026-09-16 (owner decision) — §2 adds the HDF-namespaced prop `description-label` on the `check` and impact-0 `fix` evidence entries and on the `fix` remediation, so the importer can return those descriptions exactly without reading foreign evidence or remediations as HDF's (`gxeb.2`); §4.5 replaces the empty POA&M `requirementId` with an ordered candidate list (HDF prop, FedRAMP tool-actor scanner ids, CVE facet, confirmed NIST control) and skips an item with none, with a warning (`gxeb.7`, `gjll8`); §1.3 keeps FedRAMP's own impact facet system `https://fedramp.gov` instead of moving to a NIST-listed URI no FedRAMP template uses (`gxeb.3`); §4.5 states the foreign SAR grouping rule generically — case-insensitive, confirmed at any supported revision, with optional dot-separated parts before the `_obj`/`_smt` suffix — so it depends on no single producer's id style (`gxeb.6`).
- **Revision:** 2026-09-17 (owner decisions) — §1.7.5 names the field an `empty-field`/`absent-field` marker refers to; §4.3 reads pre-ADR HDF POA&M exports through their pre-ADR mapping for the fallback window, skips with a warning a pre-ADR item whose risk has no `impacted-control-id`, and reads a POA&M document's own fields from HDF homes only when one of its risks is HDF-produced; §4.6 carries the new optional milestone `title` exactly, with `Milestone <n>` as display text when absent (`gxeb.5`, `gxeb.20`).
...
3. **Third-party vocabulary keeps its owner's namespace, matched exactly.** A prop or facet defined by NIST keeps NIST's default namespace (e.g. the resource `type` prop with value `evidence`, which the NIST metadata metaschema defines as "the resource represents evidence"). A prop defined by FedRAMP carries `https://fedramp.gov/ns/oscal` (e.g. `impacted-control-id`, `POAM-ID`, as in the FedRAMP POA&M template fixture). Owner URIs are compared byte-for-byte, and the owner's own usage decides: the SAR exporter's FedRAMP impact facet keeps system `https://fedramp.gov` (owner decision 2026-09-16). NIST's metaschema lists only `http://fedramp.gov` and `http://fedramp.gov/ns/oscal` for that facet, but FedRAMP's rev5 SAR template uses `https://fedramp.gov` on every FedRAMP facet and FedRAMP's extensions registry binds its risk-metric system to `https://fedramp.gov`; the facet `system` is typed as any URI, so the value is schema-valid. Evidence was read from a mirror of `GSA/fedramp-automation` (`oscal-compass-lab/fedramp-automation` at `76d0473eafe60676d4acbd6cd76362092c389c46`), because the GSA repository was unavailable; `gxeb.3` records it.
...
7. **Values OSCAL cannot hold.** These rules apply to every HDF prop the exporters emit; the helpers in `gxeb.3` implement them once per language, pinned by a shared case table both languages read (`hdf-converters/shared/oscal-string-cases.json`).
   ...
   5. *Field names in markers.* The value of `empty-field` or `absent-field` is the HDF JSON property name, dot-separated relative to the OSCAL object that carries the marker (e.g. `systemRef`, `version`, `checksum.value`); for props that share a `group`, it names the field within that group (e.g. `key`, `value`, `fixedInVersion`) (owner decision 2026-09-17).
...
|---|---|---|
| `fix` description | `risk.remediations[]` with `lifecycle: "recommendation"` and the text as `description` (already emitted); the remediation carries prop `description-label` = `fix` | **KEEP, refined.** Model-exact ("recommended or actual plan for addressing the risk"; lifecycle `recommendation` "such as from an assessor or tool"); the FedRAMP rev5 SAR template states the convention; fully rendered by oscal-viewer. The label lets the importer return the text as `fix` exactly; an unlabelled (foreign) remediation keeps importing as today. |
| `fix` when no risk is emitted (impact 0) | the related observation's `relevant-evidence[]`, shape as for `check`, with prop `description-label` = `fix` | **REVISE.** The current preview-prop fallback shares the `check` defects. |
...
| `check` description | the finding's related observation `relevant-evidence[]` entry `{ description: <single-line preview>, remarks: <full text> }` with prop `description-label` = `check` | **REVISE.** Observation evidence is where real documents carry procedure prose, and is surfaced by oscal-viewer and Lula. The model-pure alternative (Activity/Step) is rejected — Alternative D. |
...
`description-label` (owner decision 2026-09-16) is an HDF prop in the §1.1 namespace, named per §1.6, whose value is the HDF description label it carries (`check` or `fix`). It is new, so it has no §1.4 legacy fallback, and `gxeb.3` adds its row to the §1.5 table. The importer reads a `relevant-evidence[]` entry as a description only when it carries `description-label`, taking the full text from `remarks`, and reads a remediation labelled `fix` as the `fix` description with no added text. Evidence entries and remediations without the label are foreign and import as they did before this ADR.
...
3. **HDF-produced OSCAL carries HDF identity in HDF-namespaced props.** `hdf-requirement-id` on SAR `finding`s (already emitted, gains `ns`) and on POA&M `risk`s (new); `baseline-name` on SAR `result`s (new); override identity and scope on POA&M `risk`s per §4.6. Importers read each back into its HDF field; for HDF-produced documents these props take precedence over every other source. OSCAL `uuid`s in HDF-produced documents are never read back as HDF identity, because exporters regenerate them on every conversion. A POA&M risk is HDF-produced when it carries `override-type` in the HDF namespace; `type` is required on every override, so every HDF-produced risk carries it. During the §1.4 window, a risk carrying `override-type` without `ns` is a pre-ADR HDF export: it imports through the pre-ADR mapping — `requirementId` from the legacy `impacted-control-id` prop and status through the pre-ADR status mapping, never from a title — and that mapping is removed together with the fallback (`gxeb.14`). A pre-ADR item whose risk carries no `impacted-control-id` (a pre-ADR export of an empty `requirementId`) has no candidate, so it is skipped with a warning naming its uuid and title, as §4.5 skips a foreign item with none. A POA&M document's own fields (the §4.6 Document rows) are read from their HDF homes only when at least one of its risks is HDF-produced; otherwise the document's fields import as for a foreign POA&M (owner decisions 2026-09-17).
...
5. **Foreign documents keep object identity without inventing requirement ids.**
   - *SAR requirement ids:* a requirement id comes from `hdf-requirement-id`; else, when the finding's target names a NIST control or a part of one, from that NIST control; else from the target-id verbatim. A target names a control when, ignoring letter case, it is a control id (`<family>-<number>[.<enhancement>]`, where a number directly after the control number is always the enhancement) that `nistExists` confirms at any supported revision, optionally followed by dot-separated letter-and-digit parts and then the suffix `_obj` or `_smt`, alone or followed by `.` and one or more non-empty dot-separated parts of any characters (e.g. `ac-2.3`, `ac-2.3_obj.a`, `au-1_smt.a`, `ac-1.a.1_obj.1`, `ac-1_obj.a.1-1`; not `ac-1_objective`, `ac-1_gdn` or `ac-1_obj.`); objectives and statements group under their control, as today (owner decision 2026-09-16). The rule is catalog-agnostic: it depends only on OSCAL's part-id shape and the NIST roster, so every schema-valid target-id is accepted, and a target from any other catalog, or one that fails confirmation, becomes its own requirement verbatim. Distinct unconfirmed target ids never merge. A finding whose `target-id` is empty is invalid OSCAL and is skipped with a warning naming the finding; no requirement is named `unknown`. `nistExists` is fixed to accept every common NIST spelling and gains a Go peer (`gxeb.12`); the shape regex is no longer the test.
   ...
   - *POA&M:* an override's `requirementId` comes from the first of these that the item's risks carry (owner decision 2026-09-16): `hdf-requirement-id`; a scanner identifier FedRAMP records on the risk's tool actor (`risk.characterizations[].origin.actors[]` of type `tool`), the FedRAMP-namespace prop `plugin-id` and then `vulnerability-id`; the `cve-id` facet with system `http://cve.mitre.org`; FedRAMP `impacted-control-id` (namespace-matched, roster-confirmed). Candidates are not checked against the results the amendments will be applied to — `hdf amend verify` reports a `requirementId` those results lack. An item with no candidate is skipped with a warning naming its uuid and title, because `requirementId` is the override's key and every HDF tool that reads amendments requires it. `POAM-ID` is a tracking number and the item title is display text; neither is ever a `requirementId`. Which candidate wins when an item carries several of one kind, and how bare scanner ids from different tools stay distinct, are settled on `gxeb.7`. The only foreign POA&M in the repository is the FedRAMP template, whose scanner-identifier values are placeholders, so this placement is evidenced but real-world population is not. Each override records the item it came from in its existing `externalReferences` (ADR-0006) as `{ "sourceName": "x-oscal-poam-item", "href": "#<poam-item-uuid>", "externalId": "<POAM-ID>" }`, with `externalId` present only when the item has a `POAM-ID`. No schema change. Risks that are not HDF-produced (§4.3) keep today's status mapping.
...
6. **POA&M field round-trip contract.** HDF amendments → OSCAL POA&M → HDF returns every field below exactly. Every OSCAL home listed was checked against the OSCAL 1.2.3 POA&M schema. "Today" is the measured behaviour before this ADR. Props named here carry the HDF namespace unless stated; every prop value follows §1.7; numbers are formatted by the shared exporter number rule (`exportmap.FloatToken` and its TypeScript peer, pinned by `hdf-converters/shared/export-number-cases.json`). The pre-ADR props `completed-by` and `captured-by` are no longer emitted: task `responsible-roles` and observation `origins` replace them.
   ...
   **Override fields**
   ...
   | `milestones[]` | one risk `remediations[]` entry per milestone (`lifecycle: "planned"`) with one `tasks[]` entry: `description` → remediation `description` (exact); `title` → remediation and task `title` (exact; when absent, both carry the display text `Milestone <n>`, `n` being the milestone's 1-based position in the override, plus `absent-field` = `title` — owner decision 2026-09-17); `estimatedCompletion` → task `timing.within-date-range` `start` and `end`; `status` → task prop `milestone-status`; `completedAt` → task prop `completed-at`; `completedBy` → task `responsible-roles[]` entry (`role-id: "completed-by"`, defined in metadata `roles`) whose `party-uuids` name the party | exact for the measured example; `completedBy` carries identifier only |
