...
- **Size:** a multi-source call loads N documents, each within `HDF_MCP_MAX_SIZE`; the view is bounded by their sum and rebuilt per call. Response size is bounded as today by `limit`/`page`. The store (ADR-0012) is the answer when N stops being small.
