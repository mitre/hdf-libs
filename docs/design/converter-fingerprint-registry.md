...
Consumers (Vara upload, CLI, Bifrost MCP) lack a unified auto-detection API.
...
6. Works in browser (Vara), server (Nitro), CLI (Go), and MCP (Bifrost)
...
```typescript
// Consumer code (e.g., Vara upload endpoint):
...
## Execution Order (dependency-correct, TDD)

### Phase 0: Fix foundation (before anything else)
1. Revise `registry.ts` — rename to `ConverterFingerprint`, remove `convert` field, add `outputType`
2. Revise `fingerprint.ts` — update types, remove CSV detection
3. Update existing tests to match new types
4. Revise SARIF reference implementation
...
9. Go SARIF fingerprint as reference
10. Go `register_all.go`

### Phase 3: Converter fingerprints (swarm-parallelizable, TS+Go per batch)
11. Batch 1: JSON A-G (aws-config, cyclonedx, conveyor, deptrack, gitlab, gosec, grype)
12. Batch 2: JSON J-S (jfrog-xray, msft-*, neuvector, scoutsuite, snyk, sonarqube, splunk)
13. Batch 3: JSON T-V + OSCAL + HDF (trufflehog, twistlock, veracode, oscal-*, hdf-v1, hdf-v2)
14. Batch 4: XML (nessus, netsparker, nikto, zap, burpsuite, fortify, dbprotect, xccdf+arf, junit)
15. Batch 5: Text/special (prisma, legacyhdf, sarif confidence adjustment, exports)

### Phase 4: Integration + backward compat
16. Full-suite integration tests (every converter fixture through detectConverter)
17. Migrate gosec/snyk/zap `detectFormat()` calls to registry
18. CLI `hdf convert auto` wiring

### Phase 5: Consumer wiring (separate repos)
19. Vara: `cbx` card — upload endpoint calls detectConverter() + lazy-loads converter
20. Bifrost: MCP auto-classify tool
21. DRY the HDF type detection (separate epic)
